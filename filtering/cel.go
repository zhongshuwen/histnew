...
var ActionTraceDeclarations = cel.Declarations(
	decls.NewIdent("receiver", decls.String, nil), // zswhq.account_name::receiver
	decls.NewIdent("account", decls.String, nil),  // zswhq.account_name::account
	decls.NewIdent("action", decls.String, nil),   // zswhq.name::action
	...
	decls.NewIdent("executed", decls.Bool, nil), // action has impact on chain state; it did not fail or expire or whatever (transaction status is either EXECUTED or SOFT_FAIL with a special zswhq::onerror handler that succeeded)
