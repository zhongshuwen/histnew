...
func TestCELActivation(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			activationManifest{ct.ActionTrace(t, "zswhq:zswhq:onblock", ct.Authorizations("zswhq@active")), false, 0},
			shouldMatch,
		},
		{
			"auth match, multi, single check",
			`auth.exists(x, x == "zswhq")`,
			activationManifest{ct.ActionTrace(t, "zswhq:zswhq:onblock", ct.Authorizations("zswhq@active", "zswhq@owner", "other@active")), false, 0},
			shouldMatch,
		},
...

		{
			...
			activationManifest{ct.ActionTrace(t, "zswhq:zswhq:onblock", ct.Authorizations("zswhq@active", "zswhq@owner", "other")), false, 0},
			shouldNotMatch,
		},
		{
