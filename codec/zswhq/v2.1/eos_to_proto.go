...
func TransactionReceiptToDEOS(txReceipt *TransactionReceipt) *pbcodec.TransactionReceipt {
	receipt := &pbcodec.TransactionReceipt{
		Status:               zswhq.TransactionStatusToDEOS(txReceipt.Status),
	...
	case TransactionVariant.TypeID("transaction_id"):
		receipt.Id = txReceipt.Transaction.Impl.(zsw.Checksum256).String()
...
func PackedTransactionToDEOS(in *PackedTransaction) *pbcodec.PackedTransaction {
	...
	case PrunableDataVariant.TypeID("full_legacy"):
		...
		out.Signatures = zswhq.SignaturesToDEOS(fullLegacy.Signatures)
	...
	case PrunableDataVariant.TypeID("full"):
		...
		// out.Signatures = zswhq.SignaturesToDEOS(full.Signatures)
	...
	case PrunableDataVariant.TypeID("partial"):
		...
		// out.Signatures = zswhq.SignaturesToDEOS(partial.Signatures)
...
func TransactionTraceToDEOS(logger *zap.Logger, in *TransactionTrace, opts ...zswhq.ConversionOption) *pbcodec.TransactionTrace {
	id := in.ID.String()
...
func ActionTracesToDEOS(actionTraces []*ActionTrace, opts ...zswhq.ConversionOption) (out []*pbcodec.ActionTrace, someConsoleTruncated bool) {
...
func ActionTraceToDEOS(in *ActionTrace, execIndex uint32, opts ...zswhq.ConversionOption) (out *pbcodec.ActionTrace, consoleTruncated bool) {
