...
import (
	...
	zswhq_v2_0 "github.com/zhongshuwen/histnew/codec/zswhq/v2.0"
	zswhq_v2_1 "github.com/zhongshuwen/histnew/codec/zswhq/v2.1"
...
func LimitConsoleLength(maxByteCount int) ConsoleReaderOption {
	return consoleReaderOptionFunc(func(reader *ConsoleReader) {
		if maxByteCount > 0 {
			reader.ctx.conversionOptions = append(reader.ctx.conversionOptions, zswhq.LimitConsoleLengthConversionOption(maxByteCount))
		}
	})
}
...
func NewConsoleReader(reader io.Reader, opts ...ConsoleReaderOption) (*ConsoleReader, error) {
	l := &ConsoleReader{
		...
		close: func() {},
		ctx: &parseCtx{
			hydrator:   zswhq_v2_0.NewHydrator(zlog),
...
type parseCtx struct {
	...
	conversionOptions []zswhq.ConversionOption
}
...
func (ctx *parseCtx) recordTransaction(trace *pbcodec.TransactionTrace) error {
	...
	trace.CreationTree = zswhq.CreationTreeToDEOS(toFlatTree(creationTreeRoots...))
...
func (ctx *parseCtx) readCreateOrCancelDTrxOp(tag string, line string) error {
	...
	ctx.recordDTrxOp(&pbcodec.DTrxOp{
		...
		Transaction:   zswhq.SignedTransactionToDEOS(signedTrx),
	})
...
func (ctx *parseCtx) readDeepmindVersion(line string) (majorVersion uint64, minorVersion uint64, hydrator zswhq.Hydrator, err error) {
	...
	if majorVersion == 13 {
		return majorVersion, minorVersion, zswhq_v2_1.NewHydrator(zlog), nil
	}

	return majorVersion, minorVersion, zswhq_v2_0.NewHydrator(zlog), nil
}
...
func (ctx *parseCtx) readTrxOp(line string) error {
	...
	ctx.recordTrxOp(&pbcodec.TrxOp{
		...
		Transaction:   zswhq.SignedTransactionToDEOS(trx),
	})
