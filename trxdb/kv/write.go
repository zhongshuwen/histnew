...
import (
	...
	pbtrxdb "github.com/zhongshuwen/histnew/pb/dfuse/zswhq/trxdb/v1"
...
func (db *DB) putTransactions(ctx context.Context, blk *pbcodec.Block) error {
	for _, trxReceipt := range blk.Transactions() {
		...
		signedTrx := zswhq.SignedTransactionToDEOS(signedTransaction)
		pubKeyProto := &pbcodec.PublicKeys{
			PublicKeys: zswhq.GetPublicKeysFromSignedTransaction(db.writerChainID, signedTransaction),
		}
