...
* A working `dfuseeos` command and nodeos installed with 'deep-mind' patch (see https://github.com/zhongshuwen/histnew/blob/develop/DEPENDENCIES.md#dfuse-instrumented-zswhq-prebuilt-binaries)
...
# Plugins
plugin = zswhq::producer_plugin      # for state snapshots
plugin = zswhq::producer_api_plugin  # for state snapshots
plugin = zswhq::chain_plugin
plugin = zswhq::chain_api_plugin
plugin = zswhq::http_plugin
plugin = zswhq::db_size_api_plugin
plugin = zswhq::net_api_plugin
