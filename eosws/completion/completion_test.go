...
func TestComplete(t *testing.T) {
	...
	tests := []struct {
		...
		// SQE Multi fields
		{
			"sqe multi fields, complete field name", "data.from:zswhq data.prox", 3, nil, nil, []*mdl.SuggestionSection{
				section("query", "data.from:zswhq data.proxy:"),
			},
		},
		{
			"sqe multi fields, complete field value, empty", "data.from:zswhq account:", 3, []string{"other"}, nil, []*mdl.SuggestionSection{
				section("query", "data.from:zswhq account:other"),
			},
		},
		{
			"sqe multi fields, complete field value, partial", "data.from:zswhq account:oth", 3, []string{"other"}, nil, []*mdl.SuggestionSection{
				section("query", "data.from:zswhq account:other"),
			},
		},
		{
			"sqe multi fields, complete field value, no match", "data.from:zswhq account:aaa", 3, []string{"other"}, nil, []*mdl.SuggestionSection{
				section("query", "data.from:zswhq account:aaa"),
			},
		},
		{
